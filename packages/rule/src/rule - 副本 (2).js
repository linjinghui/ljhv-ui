      ...
      var getItemAll = that.getObj.getElementsByClassName('slider-item-value');
		...
		touch: function(){
			...
			var start = function(e){
				...
				that.startLeft = e.target.offsetLeft;
				e.target.className = that.initClassName;
			}
			var move = function(e){
				e.preventDefault();
				e.stopPropagation();
			...
			var end = function(e){
				...
				e.stopPropagation();
				var moveTouch = e.changedTouches[0];
				e.target.className += ' ' + that.animatedClassName;
				that.movePageX = moveTouch.pageX;
				that.moveX = that.movePageX - that.startPageX;
				that.setSliderValue(e.target.offsetLeft);
			}
			...
			that.getObj.addEventListener("mousemove", function (evt) {
				evt.preventDefault()
				if (isMouseDown === true) {
					evt.touches = [{ pageX: evt.pageX, pageY: evt.pageY }]
					evt.changedTouches = [{ pageX: evt.pageX, pageY: evt.pageY }]
			...
			that.getObj.addEventListener("mouseup", function (evt) {
				isMouseDown = false
				evt.touches = [{ pageX: evt.pageX, pageY: evt.pageY }]
				evt.changedTouches = [{ pageX: evt.pageX, pageY: evt.pageY }]
				end(evt)
